...
> 2026-07-18 · N150 小主机服务器监控系统

---

## 一、项目概述

WeMonitor 是一个轻量级 Node.js 监控系统，运行在 N150 小主机上，监控系统资源、服务健康状态和自定义应用指标，通过 Web Dashboard 可视化展示，并支持 UFW 防火墙管理。

### 核心原则

- 单进程运行，内存占用 < 200MB，无外部组件依赖
- 零构建步骤（EJS 服务端渲染 + Chart.js CDN + Lucide CDN）
...
N150 cron 每分钟运行 ~/wemonitor-update.sh
    查询最新 Release → 比对版本 → 下载 → 解压 → restart
    ↓
N150 ~/wemonitor/
...
N150 路径：`~/wemonitor/`，端口：`18990`，访问：`http://192.168.31.102:18990`
