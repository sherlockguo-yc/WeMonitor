...
async function getSyncStatus(jobs) {
  try {
    ...
    // No WeMonitor section at all → nothing to compare against.
    if (beginIdx < 0) {
      return {
        inSync: true,
        hasWmSection: false,
    ...
    // Only compare jobs that are actually referenced in the marker section.
    // Jobs in jobs.json that were auto-imported but never synced (or removed
    // from the marker section) don't count as a mismatch.
    const markerIds = parseMarkerIds(existingWm);
    const syncedJobs = jobs.filter(j => markerIds.has(j.id));
    const generated = generateCrontab(syncedJobs).trim();

    return {
      inSync: existingWm === generated,
      hasWmSection: true,
...
// Extract job IDs referenced in a WeMonitor marker section.
function parseMarkerIds(wmSection) {
  const ids = new Set();
  const lines = wmSection.split('\n');
