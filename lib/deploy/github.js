...
// 导致始终拿到上一版。改用 releases?per_page=1 取最近一次构建（含 prerelease）。
async function getRelease(repo) {
  return cached(`rel:${repo}`, REL_TTL, async () => {
    const arr = await ghFetch(`https://api.github.com/repos/${repo}/releases?per_page=1`);
    const r = Array.isArray(arr) ? arr[0] : arr;
    if (!r) return { version: null, tag: null, publishedAt: null, htmlUrl: null };
