...
function readQueueWorkerState() {
  try {
    ...
    const isStale = worker.status === 'working' && (
      !worker.pid || !Number.isInteger(worker.pid) ||
      now - Number(worker.updatedAt || 0) > 1800 ||
      !isProcessAlive(worker.pid)
    );
    return isStale
      ? { ...worker, pending, status: 'interrupted', message: 'worker 心跳过期或进程已退出' }
      : { ...worker, pending };
  } catch (_) {
    return null;
  }
