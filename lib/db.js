...
// 清理 metrics_hourly 历史重复数据（多次 cleaner 重启导致同一 hour_ts 多行）
// 保留每个组合最早插入的行（首次聚合数据最完整），删除后续重复
db.exec(`
  DELETE FROM metrics_hourly WHERE id NOT IN (
    SELECT MIN(id) FROM metrics_hourly GROUP BY service, metric_name, labels, hour_ts
  );
`);

// 加 UNIQUE 约束防止未来重复聚合（依赖上面清理后的数据）
db.exec(`
...
const stmts = {
  ...
  `),
  // 窄窗口聚合（每小时一次增量），INSERT OR IGNORE 配合 UNIQUE 索引避免重复插入
  aggregateMetrics: db.prepare(`
    INSERT OR IGNORE INTO metrics_hourly (service, metric_name, labels, avg_value, min_value, max_value, count, hour_ts)
    SELECT
  ...
  `),
  // 全量补齐聚合（启动时使用，一次性扫描所有未聚合的首个粒度数据）
  aggregateMetricsBefore: db.prepare(`
    INSERT OR IGNORE INTO metrics_hourly (service, metric_name, labels, avg_value, min_value, max_value, count, hour_ts)
    SELECT
      service, metric_name, labels,
      AVG(value), MIN(value), MAX(value), COUNT(*),
