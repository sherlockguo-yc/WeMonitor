...
// 添加规则
async function addRule({ port, protocol = 'tcp', comment = '', from = '' }) {
  let args;
  if (from) {
    // 限制来源: sudo ufw allow from 192.168.1.100 to any port 18990 proto tcp
    args = ['allow', 'from', from, 'to', 'any', 'port', String(port), 'proto', protocol];
  } else {
    // 任意来源: sudo ufw allow 18990/tcp
...
// 编辑规则（按编号）—— 删旧 + 加新
async function editRule(number, { port, protocol = 'tcp', comment = '', from = '' }) {
  ...
  // 添加新规则
  const addResult = await addRule({ port, protocol, comment, from });
