...
function queryMetrics(req, res) {
  ...
  const t1 = Date.now();
  if (isHourly) {
    // fromTs 减 1 小时容差，确保左边界整点 hour_ts 不被严格 >= 过滤
    // 例：from=21:08 时 21:00 hour_ts 应在范围内，但 hour_ts(21:00) < from(21:08) 会被排除
    rows = stmts.queryMetricsHourly.all(
      service, metric_name,
      fromTs - 3600000, toTs,
      maxRows
    ).map(r => ({ ...r, value: r.avg_value }));
    // UNIQUE 索引已防止重复；此处去重保留作为防御性编程
    const seen = new Map();
    for (const r of rows) {
      seen.set(r.timestamp, r);
    }
    rows = [...seen.values()];
  } else {
...
function queryMetricsBatch(req, res) {
  ...
  for (const name of metricNames) {
    ...
    if (isHourly) {
      // fromTs 减 1 小时容差，确保左边界整点 hour_ts 不被严格 >= 过滤
      rows = stmts.queryMetricsHourly.all('system', name, fromTs - 3600000, toTs, maxRows)
        .map(r => ({ ...r, value: r.avg_value }));
      // UNIQUE 索引已防止重复；此处去重保留作为防御性编程
      const seen = new Map();
      for (const r of rows) {
